...
**정답 위치를 반드시 분산하세요.** 6문항의 answer_index 가 1,2,3,4 에 고르게 흩어져야
합니다(예: 3,1,4,2,1,3). 정답을 첫 번째 선택지에 두는 습관을 의도적으로 깨십시오.
정답이 한 번호에 몰리면 학생이 글을 읽지 않고 그 번호만 찍어도 점수가 나와 진단이
성립하지 않습니다. 해설에서 선지를 가리킬 때는 '1번'처럼 번호로 쓰지 말고 내용으로
설명하세요.

**네 선택지의 길이를 비슷하게 맞추세요.** 정답만 자세히 쓰고 오답을 짧게 쓰면
글을 읽지 않아도 가장 긴 것을 고르면 맞습니다. 네 선택지의 글자 수 차이가
서로 20% 이내가 되게 하고, 특히 정답이 가장 긴 선택지가 되지 않도록 하십시오.
오답도 정답만큼 구체적으로, 그럴듯한 근거를 담아 쓰십시오 —
'~는 아니다' 같은 짧은 부정문으로 처리하지 마십시오.
