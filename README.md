...
- **테스트**: `pytest tests/` **92 passed**(진단 엔진 75 + 관리자 계정 운영 17) / 실 Postgres 포함 시 **104 passed** + 풀사이클 API 스모크 + 브라우저 E2E 완주
...
| 사용자 관리 | 역할별 사용자 목록·집계 + **계정 발급 · 비밀번호 초기화 · 활성/비활성 전환** (임시 비밀번호는 발급 직후 1회만 표시) |
...
| **보호자 동의** | 학생별 회수 현황(미회수 포함) · 회수 기록 입력 · 철회 처리 · 원본 보관 위치. 서면 동의를 시스템에서 확인 가능하게 함 |
...
| 계정 발급 | `POST /api/auth/admin/users` — 관리자만. 역할은 `student`·`parent`·`teacher`로 제한(**admin 발급 불가**). 임시 비밀번호는 서버가 난수로 만들어 응답에 1회만 반환 |
| 계정 운영 | 비밀번호 초기화·활성/비활성 전환은 관리자만. **본인 계정·admin 계정은 대상에서 차단**. 비활성화는 소프트 플래그(진단 기록 보존) |
...
| 보호자 동의 | `REQUIRE_PILOT_CONSENT=true` 시 동의 기록이 없거나 철회된 학생은 **설문·세션 생성 403**. 기본은 꺼짐 — 켠 채 배포하면 기록이 없는 검수용 계정까지 막힌다 |
...
| POST | `/api/auth/admin/users` | **관리자 전용** 계정 발급 (임시 비밀번호 1회 반환, `must_change_password` 지정 가능) |
| POST | `/api/auth/admin/users/bulk` | **관리자 전용** 파일럿 학생 계정 일괄 발급 (`elem5-001…` 자동 생성, 아이디 충돌 시 전량 취소) |
| POST | `/api/auth/admin/users/{id}/reset-password` | **관리자 전용** 비밀번호 초기화 (임시 비밀번호 재발급 + 변경 강제) |
| PATCH | `/api/auth/admin/users/{id}/active` | **관리자 전용** 계정 활성/비활성 전환 (소프트) |
| PATCH | `/api/auth/users/{id}/name` | 이름 수정 (**관리자 전용** — 파일럿 식별코드 보호) |
...
| GET | `/api/admin/consents` | 학생별 보호자 동의 현황 (미회수 포함, `missing_only`) |
| POST | `/api/admin/consents` | 동의 회수 기록 등록·갱신 (학생 1명당 1건) |
| POST | `/api/admin/consents/{id}/revoke` | 동의 철회 (행 보존, 응시 차단) |
